...
from ..utils import LoggerMixin, get_logger, settings
...
class ApifyTCGPlayerScraper(LoggerMixin):
    ...
    async def get_sold_listings(
        self, card: PokemonCard, condition: str = "Near Mint", days_back: int = 30
    ...
    async def _start_actor_run(
        self, card: PokemonCard, condition: str, days_back: int
    ) -> Optional[str]:
    ...
    async def _wait_for_results(
        self, run_id: str, max_wait_time: int = 180
    ) -> Optional[List[Dict]]:
                ...
                async with self.session.get(status_url, params=params) as response:
                    if response.status == 200:
                        ...
                        if status == "SUCCEEDED":
                            ...
                            self.logger.error(
                                "Apify run failed", run_id=run_id, status=status
                            )
                            return None
                        ...
                        self.logger.error(
                            "Error checking run status", status=response.status
                        )
                        return None
    ...
    def _analyze_sold_listings(
        self, card_name: str, condition: str, raw_data: List[Dict]
    ) -> Optional[MarketAnalysis]:
        ...
        try:
            ...
            if len(sold_listings) < 3:  # Need minimum sample size
                self.logger.warning(
                    "Insufficient sold listings", count=len(sold_listings)
                )
                return None
            ...
            price_range = {
                ...
                "q3": (
                    statistics.quantiles(prices, n=4)[2]
                    if len(prices) >= 4
                    else max(prices)
                ),
            }
            ...
            confidence = min(
                len(sold_listings) / 20, 1.0
            )  # Max confidence at 20+ sales

            # Get recent sales (last 10)
            recent_sales = sorted(
                sold_listings, key=lambda x: x.sold_date, reverse=True
            )[:10]
    ...
    def _calculate_trend(self, sold_listings: List[SoldListing]) -> str:
        ...
        try:
            ...
            older_prices = [
                listing.total_price or listing.price for listing in sorted_listings[:midpoint]
            ]
            newer_prices = [
                listing.total_price or listing.price for listing in sorted_listings[midpoint:]
            ]
