...
class CameraCapture(LoggerMixin):
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            if not self.cap.isOpened():
                self.logger.error(
                    "Failed to open camera", camera_index=self.camera_index
                )
                return False
    ...
    def run_camera_loop(self, on_frame_capture=None, on_exit=None):
        ...
        try:
            while True:
                ...
                # Handle key presses
                if self._handle_key_press(on_frame_capture, on_exit):
                    break
    ...
    def _handle_key_press(self, on_frame_capture, on_exit) -> bool:
        """Handle key press events. Returns True if should exit."""
        ...
        if key == 27:  # ESC key
            ...
            self._handle_space_press(on_frame_capture)
        
        return False

    def _handle_space_press(self, on_frame_capture) -> None:
        """Handle SPACE key press for frame capture."""
        if on_frame_capture:
    ...
    def capture_stable_frame(
        self, stabilization_frames: int = 5
    ) -> Optional[np.ndarray]:
    ...
    def _extract_card_region(
        ...
        try:
            ...
            warped = cv2.warpPerspective(
                frame, transform_matrix, (target_width, target_height)
            )
