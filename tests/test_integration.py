...
class TestFullAppIntegration:
    ...
    def setup_teardown(self):
        ...
        with (
            ...
            patch(
                "src.store.cache.card_cache.db_path", self.cache_dir / "test_cards.db"
            ),
        ):
    ...
    def mock_camera(self):
        ...
        with (
            patch.object(camera_capture, "initialize", return_value=True),
    ...
    def mock_ocr(self):
        ...
        with patch.object(ocr_extractor, "extract_card_info") as mock_extract:
            mock_extract.return_value = CardInfo(
                name="Charizard", collector_number="4", confidence=85.0
            )
            yield mock_extract
    ...
    def test_full_workflow_integration(
        ...
        scan_id = card_cache.insert_scan(
            ...
            ocr_data={"name": "Charizard", "collector_number": "4", "confidence": 85.0},
        )
        ...
        # Use card_info to avoid unused variable warning
    ...
    def test_error_handling_integration(self, mock_camera, mock_ocr):
        ...
        scan_id = card_cache.insert_scan(
            ...
            ocr_data={"name": None, "collector_number": None, "confidence": 0.0},
        )
    ...
    def test_ocr_integration(self, mock_camera):
        ...
        with patch.object(ocr_extractor, "extract_card_info") as mock_extract:
            mock_extract.return_value = CardInfo(
                name="Pikachu", collector_number="58", confidence=90.0
            )
...
class TestCLICommandsIntegration:
    ...
    async def test_run_command_integration(self):
        ...
        with (
            patch("src.capture.camera.camera_capture.initialize", return_value=True),
            patch(
                "src.capture.camera.camera_capture.get_preview_frame"
            ) as mock_preview,
            patch(
                "src.capture.camera.camera_capture.capture_stable_frame"
            ) as mock_capture,
            patch(
                "src.capture.camera.camera_capture.detect_card_region"
            ) as mock_detect,
            patch("src.capture.camera.camera_capture.release") as _,
            patch("cv2.imshow") as _,
            patch("cv2.waitKey", return_value=27),  # ESC key
            patch("cv2.destroyAllWindows") as _,
        ):
            ...
            with patch("src.ocr.extract.ocr_extractor.extract_card_info") as mock_ocr:
                ...
                with (
                    ...
                    # Mock pricer
                    with patch(
                        "src.pricing.poketcg_prices.pokemon_pricer.extract_prices_from_card"
                    ) as mock_pricer:
                        ...
                        # Mock cache
                        with (
                            patch("src.store.cache.card_cache.upsert_card"),
                            patch("src.store.cache.card_cache.upsert_prices"),
                        ):

                            # Mock CSV writer
                            with (
                                patch("src.store.writer.csv_writer.write_row"),
                                patch("cv2.imwrite"),
                            ):
...
class TestErrorRecoveryIntegration:
    """Test error recovery and resilience in the full application."""
    ...
    def test_camera_failure_recovery(self):
        ...
        # Test that the camera module can handle initialization failures gracefully
        with patch("src.capture.camera.camera_capture.initialize", return_value=False):
            ...
            result = camera_capture.initialize()
    ...
    def test_ocr_failure_recovery(self):
        ...
        # Use scan_id to avoid unused variable warning
        assert scan_id is not None
...
class TestPerformanceIntegration:
    """Test performance characteristics of the full application."""
