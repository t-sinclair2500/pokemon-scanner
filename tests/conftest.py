...
def temp_dirs():
    ...
    yield {"temp_dir": temp_dir, "output_dir": output_dir, "cache_dir": cache_dir}
...
def mock_resolver():
    ...
    with (
        patch("src.resolve.poketcg.PokemonTCGResolver.search_cards") as mock_search,
        ...
        mock_card = {
            ...
            "rarity": "Holo Rare",
            "tcgplayer": {
                "prices": {"holofoil": {"market": 100.0, "low": 80.0, "high": 120.0}}
            },
            "cardmarket": {
                "prices": {
...
def sample_card_data():
    ...
    return {
        ...
        "cardmarket": {
            "prices": {"averageSellPrice": 85.0, "lowPrice": 70.0, "highPrice": 100.0}
        },
    }
...
def sample_ocr_data():
    ...
    return {"name": "Charizard", "collector_number": "4", "confidence": 85.0}
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        ...
        if "integration" in item.name.lower() or "TestFullAppIntegration" in str(
            item.cls
        ):
        ...
        if any(
            ...
            for slow_indicator in ["performance", "bulk", "cache"]
        ):
