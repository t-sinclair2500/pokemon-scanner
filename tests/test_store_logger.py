            ...
            with patch("builtins.open", mock_open()):
                with patch.object(logger.logger, "warning") as mock_warning:
                    ...
                    mock_warning.assert_called_once_with(
                        "Invalid image format, skipping image save"
                    )
            ...
            with patch("cv2.imwrite") as mock_imwrite:
                ...
                with patch.object(logger.logger, "error") as mock_error:
                    ...
                    mock_error.assert_called_once_with(
                        "Failed to log card scan", error="Image save failed"
                    )
...
class TestNotesGeneration:
    ...
    def test_generate_notes_medium_confidence(self):
        ...
        card_info = CardInfo(
            name="Gyarados", collector_number="130/150", confidence=65.0
        )
    ...
    def test_generate_notes_normal_scan(self):
        ...
        card_info = CardInfo(
            name="Snorlax", collector_number="143/150", confidence=88.0
        )
...
class TestDetailedLogSaving:
    ...
    def test_save_detailed_log_success(self, tmp_path):
        ...
        with patch("src.store.logger.Path") as mock_path:
            ...
            card_info = CardInfo(
                name="Arcanine", collector_number="59/150", confidence=92.0
            )
            csv_row = {
        ...
        with patch("src.store.logger.Path") as mock_path:
            ...
            with patch("builtins.open") as mock_file:
                ...
                with patch.object(logger.logger, "error") as mock_error:
                    ...
                    # Should log error
                    mock_error.assert_called_once_with(
                        "Failed to get scan summary", error="File read failed"
                    )
        ...
        with patch("src.store.logger.Path") as mock_path:
            ...
            with patch.object(logger, "get_scan_summary", return_value=mock_summary):
                with patch.object(logger.logger, "error") as mock_error:
                    ...
                    # Should log error
                    mock_error.assert_called_once_with(
                        "Failed to export summary CSV", error="Failed to get summary"
                    )
