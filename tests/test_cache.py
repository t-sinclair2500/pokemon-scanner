...
class TestCacheManager:
    ...
    def test_upsert_prices(self, cache_manager, sample_card, sample_price_data):
        ...
        cached_prices = cache_manager.get_price_data_from_cache(sample_card.id, max_age_hours=24)
    ...
    def test_cache_hit_within_window(self, cache_manager, sample_card, sample_price_data):
    ...
    def test_cache_miss_expired_data(self, cache_manager, sample_card, sample_price_data):
        ...
        cached_prices = cache_manager.get_price_data_from_cache(sample_card.id, max_age_hours=0)
        
        assert cached_prices is None
    
    def test_cache_miss_missing_data(self, cache_manager):
        """Test cache miss when no data exists."""
        cached_prices = cache_manager.get_price_data_from_cache("nonexistent-card", max_age_hours=24)
        assert cached_prices is None
    ...
    def test_price_data_serialization(self, cache_manager, sample_card):
        ...
        assert cached_prices is not None
        assert cached_prices.price_sources == ["pokemontcg.io", "manual_entry", "api_fallback"]
